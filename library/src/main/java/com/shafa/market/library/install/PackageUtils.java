...
public class PackageUtils {
     ...
     * @return
     */
    public static final int install(InstallMode mode, Context context, String filePath, Handler handler) {
        int ret = INSTALL_FAILED_OTHER;

        if (null != mode) {
            if (mode.getInstallModeByUser() == InstallMode.INSTALL_MODE_SYSTEM) {
                ret = installNormal(context, filePath);

                if (ret != INSTALL_SUCCEEDED) {
                    mode.setInstallMode(InstallMode.INSTALL_MODE_SILENT);

                    if (mode.canInstallADB()) {
                        int result = ADB.install(filePath);
                        if (result == 0) {
                            ret = INSTALL_SUCCEEDED;
                        }
                    }
                    if (ret == INSTALL_FAILED_OTHER && mode.canInstallSU()) {
                        ret = installSilent(context, filePath);
                    }

                }

            } else if (mode.getInstallModeByUser() == InstallMode.INSTALL_MODE_SILENT) {
                if (mode.canInstallADB()) {
                    int result = ADB.install(filePath);
                    if (result == 0) {
                        ret = INSTALL_SUCCEEDED;
                ...
                // 重置安装模式
                if (ret != INSTALL_SUCCEEDED) {
                    mode.setInstallMode(InstallMode.INSTALL_MODE_SYSTEM);
                    ret = installNormal(context, filePath);
                }
            ...
            } else if (mode.getInstallModeByUser() == InstallMode.INSTALL_MODE_UNDEFINE) {

                if (mode.getOnceInstallMode() == InstallMode.INSTALL_MODE_SYSTEM) {
                    ret = installNormal(context, filePath);
                    // 静默安装
                } else {
                    if (mode.canInstallADB()) {
     ...
     * @return
     */
    public static final int install(InstallMode mode, Context context, DownloadInfo apk, Handler handler) {
        int ret = INSTALL_FAILED_OTHER;
        String filePath = apk.mFilePath;
        if (null != mode) {
            if (mode.getInstallModeByUser() == InstallMode.INSTALL_MODE_SYSTEM) {
                ret = installNormal(context, filePath);
                ...
                if (ret != INSTALL_SUCCEEDED) {
                    ...
                    if (mode.canInstallADB()) {
                        int result = ADB.install(filePath);
                        if (result == 0) {
                            ret=verifyADBResult(context , apk);
                        }
                    }
                    if (ret == INSTALL_FAILED_OTHER && mode.canInstallSU()) {
                        ret = installSilent(context, filePath);
                ...
                if (mode.canInstallADB()) {
                    int result = ADB.install(filePath);
                    if (result == 0) {
                        Log.d(TAG,"adb install return success!");
                        ret=verifyADBResult(context , apk);
                        //ret = INSTALL_SUCCEEDED;
                    }
                }
     ...
     * @return
     */
    private static int verifyADBResult(Context mContext , DownloadInfo apk){
//        检测是否安装成功 某些乐视盒子上面adb安装不可靠需要检测
        if (ShafaManager.getInstance(mContext).checkApkInstalledByPackageName(apk.mPackageName)) {
     ...
     * @return 0
     */
    public static int installNormal(Context context, String filePath) {
        ...
        try {
            if (file != null && file.length() > 0 && file.exists() && file.isFile()) {
                ...
                return INSTALL_ACTIVITY_START;
            }
        } catch (Exception e) {
            int ret = ADB.install(filePath);
            return ret == 0 ? INSTALL_SUCCEEDED : INSTALL_FAILED_OTHER;
        }
